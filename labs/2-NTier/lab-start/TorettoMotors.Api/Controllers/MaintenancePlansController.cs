...
public class MaintenancePlansController : ControllerBase
{
	...
	[HttpPost("{id}/renew")]
	public async Task<ActionResult<MaintenancePlanDto>> Renew(int id, [FromBody] RenewRequest request)
	{
		try
		{
			var renewed = await _maintenancePlanService.RenewAsync(id, request.NewStartDate);
			return Ok(renewed);
		}
